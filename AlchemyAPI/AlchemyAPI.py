...
class AlchemyAPI:
    ...
    def PostRequest(self, apiCall, apiPrefix, paramObject):
        ...
        status = etree.fromstring(result).find('.//status').text
        status_info = etree.fromstring(result).find('.//statusInfo').text
        if status != "OK":
            raise Exception, 'Error making API call: %s\n%s: %s' % (
                endpoint, status, status_info)
